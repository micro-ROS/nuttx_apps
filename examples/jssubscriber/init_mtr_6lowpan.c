...
// When running on nsh prompt
    ...
    sprintf(buffer,"i8sak wpan0 startpan %02x:%02x", MOTOR_PAN_ID & 0xff, MOTOR_PAN_ID >> 8); //Set the radio as an endpoint.  
    system(buffer);

    // system("i8sak set chan 11"); //Set the radio channel.
    sprintf(buffer,"i8sak set chan %d",MOTOR_CHANNEL); //Set the radio channel.   
    system(buffer);

    // system("i8sak set panid cd:ab"); //Set network PAN ID.
    sprintf(buffer,"i8sak set panid %02x:%02x", MOTOR_PAN_ID & 0xff, MOTOR_PAN_ID  >> 8); //Set network PAN ID
    system(buffer);

    sprintf(buffer,"i8sak set saddr 42:%02x",MOTOR_DEVICE_ID); // Set the short address of the radio
    system(buffer);

    sprintf(buffer, "i8sak set eaddr 00:fa:de:00:de:ad:be:%02x", MOTOR_DEVICE_ID); // TODO: This won't work on the lastest version of NuttX
    system(buffer);
