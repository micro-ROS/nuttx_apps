...
#define LOOPS_TO_STOP       5                   // Number of no command loops to STOP
                                                // TupeK motor 
...
#endif
{
    ...
    int fd;                            // TUPEK_MOTOR device descriptor
...
// Initialize 6lowpan when running on nsh prompt
	...
	do {
        ...
        motor_ctl1(&velctrl);
        // printf(" velocity_cmd sent to motor: v1 %2d, v2 %2d \n", velctrl.vel1, velctrl.vel2);
        printf(" velocity_cmd sent to motor: v2 %2d, v1 %2d \n", velctrl.vel2, velctrl.vel1);
